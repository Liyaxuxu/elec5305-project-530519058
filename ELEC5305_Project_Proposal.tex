...
GitHub Project & {\small\url{https://github.com/Liyaxuxu/elec5305-project-530519058}} \\[0.35em]
...
The proposed approach uses short-term modulation information as a simple indicator of change. If the spectral energy begins to vary rapidly, the system can react faster. When the noise is stable, it can use slower and smoother updates. Modulation-domain processing has previously been applied to speech enhancement and provides a useful basis for this design \cite{paliwal2009}. The proposed controller remains easy to interpret and does not require training a large neural network. VoiceBank-DEMAND provides paired clean and noisy recordings, making it possible to compare enhanced speech against a clean reference \cite{voicebank2017,valentini2018}.
...
\begin{enumerate}[leftmargin=1.7em,itemsep=0.25em,topsep=0.25em]
  \item \textbf{Pre-processing:} Resample and normalise each recording, divide it into overlapping Hann-windowed frames, and calculate the STFT. Suitable frame and hop sizes will be chosen using established speech-processing practice.
