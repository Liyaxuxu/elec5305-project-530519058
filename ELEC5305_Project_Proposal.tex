...
\begin{enumerate}[leftmargin=1.7em,itemsep=0.25em,topsep=0.25em]
  \item \textbf{Pre-processing:} Resample and normalise each recording, divide it into 32-ms Hann-windowed frames with an 8-ms hop, and calculate the STFT.
  ...
  \item \textbf{Noise and change estimation:} Estimate the noise power spectrum and frame SNR. The change score will combine the mean absolute difference between consecutive log-power spectra with modulation energy calculated from a 64-frame temporal DFT of subband-energy trajectories. Energy between 2 and 16 Hz will represent the short-term modulation component. The feature weights and decision threshold will be selected using development data only.
