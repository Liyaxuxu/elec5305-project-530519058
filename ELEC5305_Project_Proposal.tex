...
In this project, I will build a MATLAB system based on the short-time Fourier transform (STFT). The system will estimate the current signal-to-noise ratio (SNR) and measure how quickly the noise spectrum is changing. It will then adjust the noise-estimation speed and suppression strength frame by frame. I will compare this method with fixed spectral subtraction and fixed Wiener filtering under several noise types and SNR levels.
...
My idea is to use short-term modulation information as a simple indicator of change. If the spectral energy begins to vary rapidly, the system can react faster. When the noise is stable, it can use slower and smoother updates. This keeps the method explainable and avoids the need to train a large neural network. VoiceBank-DEMAND provides paired clean and noisy recordings, which makes it possible to compare enhanced speech against a clean reference \cite{voicebank2017,valentini2018}.
...
Experiments will use VoiceBank-DEMAND and selected DEMAND environmental noises. I will generate controlled mixtures at $-5$, $0$, $5$, and $10$~dB SNR. Speakers and noise recordings used for final testing will not be used to select parameters. This separation is important because otherwise the method could be tuned to the test examples.

The comparison will include noisy speech, fixed spectral subtraction, fixed Wiener filtering, SNR-only adaptation, and the complete modulation-aware method. I will report output SNR improvement, segmental SNR, short-time objective intelligibility (STOI) \cite{taal2011}, optional PESQ where an authorised implementation is available, and processing time. Spectrograms and short listening examples will help explain artefacts that are not obvious from a single score. I will also remove modulation detection, SNR adaptation, and smoothing one at a time to show which parts of the system are useful.

\projectsection{Expected Outcomes}

The main outcome will be a working MATLAB program that can process a noisy speech file using either a baseline method or the proposed adaptive method. I expect the adaptive method to be most useful when the background noise changes suddenly, but I will not assume that it must win in every condition. A successful project will show a repeatable improvement over at least one fixed baseline without causing a large drop in STOI or excessive processing time.
