...
1. **Pre-processing:** Resample and normalise each recording, divide it into overlapping Hann-windowed frames, and calculate the STFT. Suitable frame and hop sizes will be chosen using established speech-processing practice.
...
3. **Noise and change estimation:** Estimate the noise power spectrum and frame SNR. A change score will combine differences between consecutive log-power spectra with short-term modulation energy from subband-energy trajectories. The analysis range, feature weights, and decision threshold will be selected using relevant literature and development data only.
...
Experiments will use VoiceBank-DEMAND and selected DEMAND environmental noises. Controlled mixtures will cover several noise categories and a range of challenging and moderate SNR conditions. Separate development and test sets will be used: the development set will guide parameter selection, while the final evaluation will use unseen speakers and noise recordings. This separation prevents the method from being tuned to the test examples.
